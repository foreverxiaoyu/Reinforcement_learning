...
gamma = 1.0  # 折扣因子
actor_lr = 0.001  # 策略网络的学习率
critic_lr = 0.01  # 价值网络的学习率
...
for i in range(num_episodes):
    ...
    # 打印回合信息
    print(f'iter:{i}, episode_return:{episode_return},episode_std:{np.std(return_list[-100:])}')
...
plt.title(f'PPO,actor_lr={actor_lr},critic_lr={critic_lr},gamma={agent.gamma}')
plt.xlabel('Episode')
plt.ylabel('Return')
covergence_episode = calculate_convergence_speed(return_list, convergence_threshold,
                                                 window_size)  # 收敛速度, 100个episode的标准差小于60的最后一个episode
mean_reward, std_reward, mae_reward = calculate_stability(return_list, window_size)  # 平均值和标准差以及平均绝对误差
plt.axhline(mean_reward, color='r', linestyle='--', label=f'last100mean_y={mean_reward}')
plt.axvline(covergence_episode, color='g', linestyle='--',
            label=f'covergence_x={covergence_episode}')
